...
int main(){
    ...
    bool is_neg_cycle_exists=false;
    for(int i=1; i<=n; i++){//iteration to update all distance (max n-1)
        for(int node=1; node<=n; node++){//selecting all node to explore
            for(intpr adj_node:adj_list[node]){//exploring all adjacent node
                ...
                if(d[u]+w<d[v]){
                    d[v]=d[u]+w;
                    if(i==n){
                        is_neg_cycle_exists=true;
                    }
                }
            }
        }
    }
    if(is_neg_cycle_exists){
        cout<<"Negative cycle esists.\n";
        return 0;
    }
